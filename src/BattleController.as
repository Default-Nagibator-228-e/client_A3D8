package
{
   ...
   import alternativa.network.NSocket;
   ...
   public class BattleController implements INetworkListener
   {
      ...
      public static var activeTanks:Dictionary = new Dictionary();
      ...
      public var battle:BattlefieldModelActivator = new BattlefieldModelActivator();
      
      private var preparedPos:Vector3d;
      
      private var myTank:ClientTank;
      
      private var ctfObj:ClientObject = new ClientObject("ctfModel",null,"ctfModelObj",null);
      
      private var serverInventoryModel:ServerInventoryModel = new ServerInventoryModel();
      
      private var serverMinesModel:ServerBattleMinesModel = new ServerBattleMinesModel();
	  ...
	  private var sock:NSocket;
	  
	  private var tim:Timer;
	  
	  private var gggw:Boolean = false;
	  
	  private var xxx1:TanksWarfareActivator = new TanksWarfareActivator();
	  
	  private var xxx2:BattlefieldGUIActivator = new BattlefieldGUIActivator();
	  
	  private var xxx3:BattlefieldSharedActivator = new BattlefieldSharedActivator();
	  
	  private var xxx5:TankExplosionModel = new TankExplosionModel();
      
      public function BattleController()
      {
         super();
         xxx1.start(Main.osgi);
         xxx2.start(Main.osgi);
         xxx3.start(Main.osgi);
         this.battle.start(Main.osgi);
         Main.osgi.registerService(IBattleField,this.battle.bm);
         Main.osgi.registerService(ITankExplosionModel, xxx5);
      }
	  ...
	  private function onconn(e:Event) : void {
		...
		private function starttest() : void {
			...
			sock.addEventListener(IOErrorEvent.IO_ERROR,function():void{});
			sock.addEventListener(SecurityErrorEvent.SECURITY_ERROR, function():void{});
			sock.addEventListener(Event.CONNECT,onconn);
			sock.init();
			time = getTimer();
			sock.connect("127.0.0.1", 4894);
		}
      ...
      public function onData(data:Command) : void
      {
         ...
         catch(e:Error)
         {
            ...
            alert.text = "Произошла ошибка: " + s;
            Main.systemUILayer.addChild(alert);
			throw new Error(s);
         }
      }
	  
	  private function startpi(e:Event) : void
      {
      ...
      {
		 xxx1 = new TanksWarfareActivator();
		 xxx1.start(Main.osgi);
		 xxx2 = new BattlefieldGUIActivator()
         xxx2.start(Main.osgi);
         //xxx3.start(Main.osgi);
         //this.battle.start(Main.osgi);
         //Main.osgi.registerService(IBattleField,this.battle.bm);
         activeTanks = new Dictionary();
		 xxx5 = new TankExplosionModel();
         Main.osgi.registerService(ITankExplosionModel, xxx5);
      ...
      public function destroy() : void
      {
		 ...
		 if (this.battle.bm != null)
		 {
			this.battle.bm.spectatorMode = false;
			this.battle.bm.objectUnloaded(null);
		 }
		 if(IMaterialRegistry(Main.osgi.getService(IMaterialRegistry)) != null) IMaterialRegistry(Main.osgi.getService(IMaterialRegistry)).clear();
		 if(ITextureMaterialRegistry(Main.osgi.getService(ITextureMaterialRegistry)) != null) ITextureMaterialRegistry(Main.osgi.getService(ITextureMaterialRegistry)).clear();
		 if (IMaterialSequenceRegistry(Main.osgi.getService(IMaterialSequenceRegistry)) != null) IMaterialSequenceRegistry(Main.osgi.getService(IMaterialSequenceRegistry)).clear();
		 xxx1.stop(Main.osgi);
		 xxx1 = null;
		 xxx2.stop(Main.osgi);
		 xxx2 = null;
		 //xxx3.stop(Main.osgi);
		 //this.battle.stop(Main.osgi);
		 //Main.osgi.unregisterService(IBattleField);
		 Main.osgi.unregisterService(ITankExplosionModel);
		 xxx5 = null;
