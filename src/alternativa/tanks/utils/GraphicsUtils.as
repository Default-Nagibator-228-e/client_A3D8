   ...
   public class GraphicsUtils
   {
      ...
      public static function getTextureAnimation(param1:TextureMaterialRegistry, param2:BitmapData, frameWith:int, frameHeight:int, maxFrames:int = 0, param6:Boolean = true) : TextureAnimation
      {
         ...
         while(_loc7_ < _loc6_)
         {
			...
			var ds:BitmapTextureResource = new BitmapTextureResource(_loc8_);
			ds.upload(Game.context);
			var op:TextureMaterial = new TextureMaterial(ds);
			op.alphaThreshold = 1;
            _loc2_.push(op);
            _loc7_++;
         }
		 if (_loc2_.length == 0)
		 {
			var ds1:BitmapTextureResource = new BitmapTextureResource(param2);
			ds1.upload(Game.context);
			 var op:TextureMaterial = new TextureMaterial(ds1);
			 op.alphaThreshold = 1;
            _loc2_.push(op);
			var op1:TextureMaterial = new TextureMaterial(ds1);
			op1.alphaThreshold = 1;
            _loc2_.push(op1);
		 }
