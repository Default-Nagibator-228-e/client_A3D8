package alternativa.tanks.models.weapon.shaft
{
   ...
   public class ShaftModel implements IGunModelBase, IObjectLoadListener, IWeaponController
   {
	  ...
	  public function fly(p:Vector3) : void
		{
			cu = new FlyCameraController(battlefieldModel.bfData.viewport.camera,new Object3D());
			cu.init1(p, 200);
      ...
      public function update(time:int, deltaTime:int) : Number
      {
			...
			if (time >= nextShTime && this._triggerPressed)
			{
				 ...
				 if (!et.visible)
				 {
					...
					et.visible = true;
					var re:BitmapData = ((localTankData.tank.title.sprite.material as TextureMaterial).diffuseMap as BitmapTextureResource).data;
					et.bitmapData = re;
