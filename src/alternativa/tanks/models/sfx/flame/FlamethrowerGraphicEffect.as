package alternativa.tanks.models.sfx.flame
{
   ...
   public class FlamethrowerGraphicEffect extends PooledObject implements IGraphicEffect
   {
	  ...
	  private var numParticles1:int = 0;
	  ...
	  private var light:OmniLight = new OmniLight(16745512, 350, 750);
      ...
      public function init(shooterData:TankData, range:Number, coneAngle:Number, maxParticles:int, particleSpeed:Number, muzzleLocalPos:Vector3, turret:Object3D, sfxData:FlamethrowerSFXData, collisionDetector:TanksCollisionDetector, weakeningModel:IWeaponWeakeningModel) : void
      {
         ...
         this.numFrames = sfxData.data.material.length;
		 ...
		 parti = new Partic(this.sfxData.shot);
		 parti.sprite.geometry.upload(Game.context);
      }
      ...
      public function play(millis:int, camera:GameCamera) : Boolean
      {
		 ...
		 //light.visible = this.numParticles > 0?true:false;
         dt = 0.001 * millis;
	  ...
	  private function addShot() : void
      {
			var barrelLength:Number = NaN;
			 ...
			 if(!this.collisionDetector.intersectRayWithStatic(barrelOrigin,gunDirection,CollisionGroup.STATIC,this.localMuzzlePosition.y + offset,null,intersection))
			 {
				...
				xAxis.y = matrix.e;
				xAxis.z = matrix.i;
				 parti.currFrame = Math.random() * this.numFrames;
				 ...
				 parti.velocity.x = this.particleSpeed * gunDirection.x;
				 parti.velocity.y = this.particleSpeed * gunDirection.y;
				 parti.velocity.z = this.particleSpeed * gunDirection.z;
				 parti.velocity.vAdd(this.shooterData.tank.state.velocity);
				 parti.distance = offset;
				 parti.sprite.x = globalMuzzlePosition.x + offset * gunDirection.x;
				 parti.sprite.y = globalMuzzlePosition.y + offset * gunDirection.y;
				 parti.sprite.z = globalMuzzlePosition.z + offset * gunDirection.z;
				 this.container.addChild(parti.sprite);
			 }
      }
      ...
      private function addParticle(globalMuzzlePosition:Vector3, direction:Vector3, gunAxisX:Vector3, offset:Number) : void
      {
		 ...
		 if (numParticles1 == 0)
		 {
			particle.light = light.clone() as OmniLight;
			particle.addChild(particle.light);
			numParticles1 = 8;
		 }
		 numParticles1--;
...
class Particle extends AnimatedSprite3D
{
   ...
   public function dispose() : void
   {
	  ...
	  //if(light != null) light::removeFromParent();
	  light = null;
      material = null;
      pool[++poolIndex] = this;
   }
   ...
   {
	  if (this.currFrame >= aData.material.length) this.currFrame -= aData.material.length;
