   ...
   public class TankResourceLoader
   {
      ...
      private function parse(e:Event) : void
      {
         ...
         if(config.id == "MODELS")
         {
            ...
            {
               this.queue.push(new TankResource(null,obj.name,null,obj.src));
               this.length++;
            }
            this.loadQueue();
         }
         ...
         loader.addEventListener(Event.COMPLETE,function(e:Event):void
         {
            ...
            for each(obj in parser.objects)
            {
               if(obj.name.split("0")[0] == "muzzle")
               {
				  ...
				  (obj as Mesh).setMaterialToAllSurfaces(null);
               }
            ...
            tnk = new TankResource(Mesh(parser.objects[0]),str.id,parser.objects,null,muzzles,flagMount,turretMount);
            list.add(tnk);
            if(length != 1)
            {
               length--;
