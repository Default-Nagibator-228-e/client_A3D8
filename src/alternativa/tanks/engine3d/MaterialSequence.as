package alternativa.tanks.engine3d
{
   ...
   public class MaterialSequence
   {
      ...
      public function MaterialSequence(registry:IMaterialSequenceRegistry, sourceImage:BitmapData, frameWidth:int, createMirroredFrames:Boolean, disposeSourceBitmap:Boolean, useMipMapping:Boolean, mipMapResolution:Number)
      {
         ...
         (TanksTextureMaterial(this.materials[0]).diffuseMap as BitmapTextureResource).data = this.getFrameImage(0);
		 (TanksTextureMaterial(this.materials[0]).diffuseMap as BitmapTextureResource).upload(Game.context);
      }
      ...
      public function tick() : Boolean
      {
         ...
         if(createMirroredFrames)
         {
            if((frameIndex & 1) == 1)
            {
               ...
               (material.diffuseMap as BitmapTextureResource).data = this.getMirroredImage((previousMaterial.diffuseMap as BitmapTextureResource).data);
			   //(material.diffuseMap as BitmapTextureResource).upload(Game.context);
            }
            else
            {
               (material.diffuseMap as BitmapTextureResource).data = this.getFrameImage(frameIndex >> 1);
			   //(material.diffuseMap as BitmapTextureResource).upload(Game.context);
            }
         }
         else
         {
            (material.diffuseMap as BitmapTextureResource).data = this.getFrameImage(frameIndex);
			//(material.diffuseMap as BitmapTextureResource).upload(Game.context);
         }
         return false;
      }
